...
if (isset($_POST['reg_user'])) {
  ...
  if($rnum==0){
    $stmt->close();
    $INSERT = "INSERT INTO registration (First_name,Last_name, Email_Id, Enter_password) VALUES (?, ?, ?, ?)";
    $stmt = $db->prepare($INSERT);
    $stmt->bind_param("ssss", $First_name,$Last_name, $Email_Id, $Enter_password,);
    $stmt->execute();
  }
  else
  {
  ...
  // Finally, register user if there are no errors in the fo
